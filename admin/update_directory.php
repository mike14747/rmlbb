...
if (isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
    ...
    if ($t_errors > 0) {
        ...
        if ($c_errors > 0) {
            echo '<p class="text-danger">The conference field has been left blank or is not in the correct format. It must either be AC or NC (and it is case sensitive).</p>';
        }
        if ($_POST['teamname'] == '') {
            echo '<p class="text-danger">The teamname field has been left blank.</p>';
        }
        if ($e_errors > 0) {
            echo '<p class="text-danger">One or more of the non-blank email fields is not in the correct format.</p>';
        }
        echo '<p class="mt-3"><a href="javascript:history.go(-1);">back</a></p>';
    } elseif ($t_errors == 0) {
        ...
        echo '<p><b>The Manager Directory has been updated with the following:</b></p>';
        echo '<p>';
